...
const SuperAdminModal: React.FC<{
  ...
  const handleSave = () => {
    ...
    // 如果展开了修改密码区域且输入了新密码，则进行校验
    let newPwdToSave: string | undefined = undefined;

    if (showPasswordSection) {
       if (!passwords.old) {
           setError('请输入原密码以确认身份');
           return;
       }
       if (passwords.old !== currentRealPassword) {
           setError('原密码错误，验证失败');
           return;
       }
       if (passwords.new) {
    ...
    <div className="fixed inset-0 z-[160] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fadeIn">
      <div className={`w-full max-w-md rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-slideInUp border ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
        ...
        <div className="p-6 space-y-5 overflow-y-auto max-h-[70vh] custom-scrollbar">
          ...
          <div>
            <label className={labelClass}>超级管理员账号 (不可修改)</label>
            <div className={`flex items-center gap-2 px-4 py-2.5 rounded-lg border opacity-60 cursor-not-allowed ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-gray-100 border-gray-200'}`}>
          ...
          <div className={`p-4 rounded-xl border transition-all ${isDark ? 'bg-slate-900/50 border-slate-700' : 'bg-gray-50 border-gray-200'}`}>
             <div className="flex items-center justify-between mb-2">
                 <div className="flex items-center gap-2">
                     <Key size={16} className={isDark ? 'text-slate-400' : 'text-slate-600'} />
                     <span className={`text-sm font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>修改登录密码</span>
                 </div>
                 <label className="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" className="sr-only peer" checked={showPasswordSection} onChange={e => setShowPasswordSection(e.target.checked)} />
        ...
        <div className={`px-6 py-4 border-t flex justify-end gap-3 ${isDark ? 'border-slate-700 bg-slate-800' : 'border-gray-100 bg-gray-50'}`}>
           <button onClick={onClose} className={`px-4 py-2 rounded-lg text-sm font-bold ${isDark ? 'text-slate-400 hover:bg-slate-700' : 'text-slate-600 hover:bg-gray-200'}`}>取消</button>
           <button onClick={handleSave} className="px-6 py-2 bg-blue-600 text-white rounded-lg text-sm font-bold hover:bg-blue-700 shadow-lg shadow-blue-500/20 transition-all active:scale-95">保存设置</button>
        </div>
      </div>
    </div>
  );
...
const UserModal: React.FC<{
  ...
  // Enforce permissions when role changes
  useEffect(() => {
    if (formData.role && ROLE_PERMISSIONS[formData.role]) {
       setFormData(prev => ({
         ...prev,
         permissions: ROLE_PERMISSIONS[prev.role as string]
       }));
    }
  }, [formData.role]);
    ...
    <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fadeIn">
      <div className={`w-full max-w-lg rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-slideInUp border ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
        ...
        <div className="p-6 space-y-5 overflow-y-auto max-h-[70vh] custom-scrollbar">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={labelClass}>登录账号 (Username)</label>
              <input 
                className={inputClass} 
                value={formData.username} 
                onChange={e => setFormData({...formData, username: e.target.value})}
                placeholder="system_user"
                disabled={!!user.id} // Cannot change username after creation
              />
            </div>
            <div>
              <label className={labelClass}>显示名称 (Display Name)</label>
              <input 
                className={inputClass} 
                value={formData.displayName} 
                onChange={e => setFormData({...formData, displayName: e.target.value})}
          ...
          {!user.id && (
             <div>
                ...
                <div className={`flex items-center gap-2 px-4 py-2.5 rounded-lg border ${isDark ? 'bg-slate-900/50 border-slate-700' : 'bg-gray-50 border-gray-300'}`}>
                ...
                <p className="text-[10px] mt-1 opacity-50">默认密码为 123456，用户首次登录需修改</p>
             </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-4">
             <div>
                ...
                <div className="relative">
                    ...
                    <input className={`${inputClass} pl-9`} value={formData.email || ''} onChange={e => setFormData({...formData, email: e.target.value})} placeholder="example@grid.com" />
                </div>
             </div>
             <div>
                <label className={labelClass}>联系电话</label>
                <div className="relative">
                    <Phone size={14} className="absolute left-3 top-3 opacity-30" />
                    <input className={`${inputClass} pl-9`} value={formData.phone || ''} onChange={e => setFormData({...formData, phone: e.target.value})} placeholder="138..." />
                </div>
             </div>
          </div>
          ...
          {/* Role Selection */}
          <div>
            <label className={labelClass}>用户角色与状态</label>
            <div className="flex gap-4">
                   ...
                   {['admin', 'operator', 'viewer'].map(r => (
                       ...
                       `}>
                           <input 
                              ...
                              className="hidden"
                           />
                           <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${formData.role === r ? 'border-blue-500' : 'border-slate-400'}`}>
                               {formData.role === r && <div className="w-2 h-2 bg-blue-500 rounded-full"></div>}
                           </div>
                           <div>
                               <div className={`text-sm font-bold ${isDark ? 'text-white' : 'text-slate-800'}`}>
                                   {r === 'admin' ? '系统管理员' : r === 'operator' ? '运维操作员' : '普通观察员'}
                               </div>
                           </div>
                       </label>
                   ))}
               ...
               <div className="w-1/3">
                   <label className={`flex flex-col items-center justify-center h-full rounded-lg border cursor-pointer transition-all
                        ${formData.status === 'active' 
                          ? (isDark ? 'bg-green-500/10 border-green-500 text-green-500' : 'bg-green-50 border-green-500 text-green-600') 
                          : (isDark ? 'bg-slate-800 border-slate-600 text-slate-400' : 'bg-gray-100 border-gray-300 text-slate-500')}
                   `}>
                       <input 
                          type="checkbox" 
                       ...
                       <span className="font-bold text-sm">{formData.status === 'active' ? '账号启用' : '账号禁用'}</span>
                   </label>
               </div>
            </div>
          </div>
          ...
          <div>
            <label className={labelClass}>模块访问权限 (由角色自动分配)</label>
            <div className="grid grid-cols-2 gap-3">
                {AVAILABLE_PERMISSIONS.map(perm => {
                    ...
                    return (
                        <div 
                            key={perm.key}
                            className={`flex items-center gap-3 p-3 rounded-lg border transition-all cursor-default
                                ${hasPerm 
                                    ? (isDark ? 'bg-blue-500/20 border-blue-500/50' : 'bg-blue-50 border-blue-300') 
                                    : (isDark ? 'bg-slate-900 border-slate-700 opacity-40' : 'bg-white border-gray-200 opacity-40')}
                            `}
                        >
                            <div className={`p-1.5 rounded ${hasPerm ? 'bg-blue-500 text-white' : 'bg-slate-500/20'}`}>
                                <perm.icon size={14} />
                            </div>
                            <span className={`text-xs font-bold ${hasPerm ? (isDark ? 'text-blue-300' : 'text-blue-700') : ''}`}>{perm.label}</span>
                            {hasPerm && <Check size={14} className="ml-auto text-blue-500" />}
                        </div>
                    );
                })}
            </div>
        ...
        <div className={`px-6 py-4 border-t flex justify-end gap-3 ${isDark ? 'border-slate-700 bg-slate-800' : 'border-gray-100 bg-gray-50'}`}>
           <button onClick={onClose} className={`px-4 py-2 rounded-lg text-sm font-bold ${isDark ? 'text-slate-400 hover:bg-slate-700' : 'text-slate-600 hover:bg-gray-200'}`}>取消</button>
           <button onClick={() => onSave(formData)} className="px-6 py-2 bg-blue-600 text-white rounded-lg text-sm font-bold hover:bg-blue-700 shadow-lg shadow-blue-500/20 transition-all active:scale-95">保存配置</button>
        </div>
      </div>
    </div>
  );
...
const SystemSettings: React.FC<SystemSettingsProps> = ({ isDark }) => {
  ...
  useEffect(() => {
    localStorage.setItem('sys_super_admin', JSON.stringify(superAdmin));
  }, [superAdmin]);
  ...
  const handleSaveSuperAdmin = (updatedUser: User, newPassword?: string) => {
      ...
      if (newPassword) {
          ...
          localStorage.setItem('sys_sa_pwd', newPassword);
  ...
  return (
    <div className={`w-full h-full flex flex-col p-6 overflow-hidden ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>
      ...
      <SuperAdminModal
        ...
        isDark={isDark}
        admin={superAdmin}
        currentRealPassword={superAdminPassword}
      ...
      <div className="flex justify-between items-center mb-8 flex-shrink-0">
        <div>
          <h1 className={`text-2xl font-black mb-1 flex items-center gap-3 ${isDark ? 'text-white' : 'text-slate-900'}`}>
            <UserCog className="text-blue-500" /> 系统用户管理
          </h1>
          <p className="text-sm opacity-60 font-medium">配置超级管理员信息，并为普通用户分配登录账号和访问权限</p>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 h-full overflow-hidden">
        ...
        <div className={`lg:w-1/3 flex flex-col rounded-2xl border p-6 relative overflow-hidden flex-shrink-0 ${isDark ? 'bg-gradient-to-br from-slate-900 to-slate-800 border-slate-700' : 'bg-white border-gray-200 shadow-sm'}`}>
            <div className={`absolute top-0 right-0 p-3 rounded-bl-2xl border-b border-l text-xs font-bold flex items-center gap-2 ${isDark ? 'bg-purple-500/10 border-slate-700 text-purple-400' : 'bg-purple-50 border-gray-100 text-purple-600'}`}>
                <Shield size={14} /> Root Account
            </div>

            <div className="flex flex-col items-center mt-6 mb-6">
                <div className="w-24 h-24 rounded-full bg-gradient-to-tr from-purple-500 to-blue-500 p-[2px] mb-4 shadow-lg shadow-purple-500/20">
                    <div className={`w-full h-full rounded-full flex items-center justify-center ${isDark ? 'bg-slate-900' : 'bg-white'}`}>
                        <span className="text-3xl font-black bg-gradient-to-r from-purple-500 to-blue-500 bg-clip-text text-transparent">SA</span>
                    </div>
                </div>
                <h2 className={`text-xl font-black ${isDark ? 'text-white' : 'text-slate-900'}`}>{superAdmin.displayName}</h2>
                <p className="text-sm opacity-50 font-mono">@{superAdmin.username}</p>
            </div>

            <div className="space-y-4 mb-8">
                <div className={`p-4 rounded-xl border flex items-center gap-4 ${isDark ? 'bg-black/20 border-slate-700' : 'bg-gray-50 border-gray-100'}`}>
                    <div className="p-2 rounded-lg bg-blue-500/10 text-blue-500"><Mail size={18}/></div>
                    <div className="flex-1 min-w-0">
                        <div className="text-[10px] opacity-40 font-bold uppercase">安全邮箱</div>
                        <div className="text-sm font-medium truncate">{superAdmin.email}</div>
                    </div>
                </div>
                <div className={`p-4 rounded-xl border flex items-center gap-4 ${isDark ? 'bg-black/20 border-slate-700' : 'bg-gray-50 border-gray-100'}`}>
                    <div className="p-2 rounded-lg bg-purple-500/10 text-purple-500"><Phone size={18}/></div>
                    <div className="flex-1 min-w-0">
                        <div className="text-[10px] opacity-40 font-bold uppercase">安全手机</div>
                        <div className="text-sm font-medium truncate">{superAdmin.phone}</div>
                    </div>
                </div>
            </div>
            ...
            <div className="mt-auto">
                <button 
                  onClick={() => setIsSaModalOpen(true)}
                  className={`w-full py-3 rounded-xl border font-bold flex items-center justify-center gap-2 transition-all active:scale-95
                    ${isDark ? 'bg-slate-800 border-slate-600 hover:bg-slate-700' : 'bg-gray-50 border-gray-200 hover:bg-gray-100 text-slate-700'}`}
                >
                    <Edit size={16} /> 编辑管理员资料
                </button>
            </div>
        </div>
        ...
        <div className={`flex-1 flex flex-col rounded-2xl border overflow-hidden ${isDark ? 'bg-slate-900/40 border-slate-700' : 'bg-white border-gray-200 shadow-sm'}`}>
            <div className={`p-4 border-b flex justify-between items-center ${isDark ? 'border-slate-700 bg-slate-800/50' : 'border-gray-100 bg-gray-50/50'}`}>
                ...
                <button 
                    ...
                    className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-bold shadow-lg shadow-blue-500/20 active:scale-95 transition-all"
                >
                    <Plus size={16} /> 新增用户
                </button>
            </div>
            ...
            <div className="flex-1 overflow-auto custom-scrollbar">
                <table className="w-full text-left text-sm border-collapse">
                    <thead className={`sticky top-0 z-10 ${isDark ? 'bg-slate-800 text-slate-400' : 'bg-gray-50 text-slate-500'}`}>
                        <tr>
                            <th className="px-6 py-4 font-bold uppercase tracking-wider text-[10px]">用户身份</th>
                            <th className="px-6 py-4 font-bold uppercase tracking-wider text-[10px]">角色权限</th>
                            <th className="px-6 py-4 font-bold uppercase tracking-wider text-[10px]">访问模块</th>
                            <th className="px-6 py-4 font-bold uppercase tracking-wider text-[10px]">状态 / 最近登录</th>
                            <th className="px-6 py-4 font-bold uppercase tracking-wider text-[10px] text-right">操作</th>
                        </tr>
                    </thead>
                                ...
                                <tr key={u.id} className={`transition-colors ${isDark ? 'hover:bg-white/5' : 'hover:bg-gray-50'}`}>
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className={`w-9 h-9 rounded-lg flex items-center justify-center font-bold text-sm ${isDark ? 'bg-slate-700 text-slate-300' : 'bg-slate-200 text-slate-600'}`}>
                                                {u.displayName.substring(0, 1)}
                                            </div>
                                            <div>
                                                <div className="font-bold">{u.displayName}</div>
                                                <div className="text-[10px] opacity-50 font-mono">@{u.username}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className={`px-2 py-1 rounded-md text-[10px] font-black border uppercase ${roleBadge.class}`}>
                                            {roleBadge.label}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="flex flex-wrap gap-1 max-w-[200px]">
                                            {u.permissions.slice(0, 3).map(p => {
                                                const permInfo = AVAILABLE_PERMISSIONS.find(ap => ap.key === p);
                                                return permInfo ? (
                                                    <div key={p} className={`p-1 rounded ${isDark ? 'bg-blue-500/20 text-blue-300' : 'bg-blue-50 text-blue-600'}`} title={permInfo.label}>
                                                        <permInfo.icon size={12} />
                                                    </div>
                                                ) : null;
                                            })}
                                            {u.permissions.length > 3 && (
                                                <span className="text-[10px] opacity-50 font-bold p-1">+{u.permissions.length - 3}</span>
                                            )}
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="flex flex-col">
                                            ...
                                            <div className="text-[10px] opacity-40">{u.lastLogin || '从未登录'}</div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex justify-end gap-2">
                                            <button 
                                                ...
                                                className={`p-2 rounded-lg transition-all ${isDark ? 'hover:bg-slate-700 text-blue-400' : 'hover:bg-gray-100 text-blue-600'}`}
                                            >
                                                <Edit size={16} />
                                            </button>
                                            <button 
                                                onClick={() => handleDeleteUser(u.id)}
                                                className={`p-2 rounded-lg transition-all ${isDark ? 'hover:bg-slate-700 text-red-400' : 'hover:bg-gray-100 text-red-600'}`}
                                            >
                                                <Trash2 size={16} />
                                            </button>
                                        </div>
                ...
                {filteredUsers.length === 0 && (
                    <div className="py-20 text-center opacity-30">
                        ...
                        <p className="text-sm font-bold">没有找到用户</p>
                    </div>
                )}
            </div>
        </div>
