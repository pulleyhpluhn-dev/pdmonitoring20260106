...
function App() {
  ...
  const [statusCollapsed, setStatusCollapsed] = useState(false); // New state for right sidebar
  ...
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth < 1280) {
        ...
        setStatusCollapsed(true); // Auto collapse right sidebar on smaller screens
      }
    };
    if (window.innerWidth < 1280) {
        setSidebarCollapsed(true);
  ...
  const handleSensorSelect = (id: string, name?: string) => {
          ...
          {currentView === 'dashboard' ? (
            ...
            <>
              ...
              <div className="flex-1 min-h-0 w-full overflow-hidden">
                <TrendAnalysis isDark={isDark} sensorName={activeSensorName} sensorId={activeSensorId} />
              </div>
            </>
          )}
        </div>
